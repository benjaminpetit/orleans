    ...
    public static class AzureTableGrainDirectoryServiceCollectionExtensions
    {
            ...
            Action<OptionsBuilder<AzureTableGrainDirectoryOptions>> configureOptions)
        {
            ...
            services
                ...
                .AddGrainDirectory(name, (sp, name) => ActivatorUtilities.CreateInstance<AzureTableGrainDirectory>(sp, sp.GetOptionsByName<AzureTableGrainDirectoryOptions>(name as string)))
                .AddSingleton<ILifecycleParticipant<ISiloLifecycle>>(s => (ILifecycleParticipant<ISiloLifecycle>)s.GetGrainDirectory(name));
